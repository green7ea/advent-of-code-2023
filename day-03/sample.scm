...
(define (matches line regex)
  (define (match-recursive line regex acc start)
    (let ((full-match (string-match regex line start)))
      (if full-match
          (let ((match (vector-ref full-match 1)))
            (match-recursive line
                           regex
                           (cons match acc)
                           (cdr match)))
          acc)))

  (let ((positions (reverse (match-recursive line regex '() 0))))
    (map (lambda (pos)
           (cons (substring line (car pos) (cdr pos)) pos))
         positions)))

(define (line->number-list line)
  (matches line "[0-9]+"))

(define (line->symbol-list line)
  (matches line "[^0-9\\.]+"))
...
(define (overlaps? a b)
  (if (or (null? a) (null? b))
      ...
      (let ((start-a (cadr a))
        ...
        (and (<= start-a end-a)
             (<= start-b end-b)
             (not (or (> start-b end-a)
                      (> start-a end-b)))))))

(define (any? predicate search-list)
  (if (null? search-list)
      #f
      (if (predicate (car search-list))
          #t
          (any? predicate (cdr search-list)))))

(define (overlaps-any? one many)
  (any? (lambda (x)
          (let ((res (overlaps? one x)))
            (display `(overlaps? ,one ,x ,res))
            (display "\n")
            res))
       many))
...
(define (numbers-next-to-sym num-lines sym-lines)
  (if (null? num-lines)
      '()
      (cons (filter (lambda (num)
                      (or (overlaps-any? num (get sym-lines 0))
                          (overlaps-any? num (get sym-lines 1))
                          (overlaps-any? num (get sym-lines 2))))
                    (car num-lines))
            (numbers-next-to-sym (cdr num-lines) (cdr (or sym-lines '(1)))))))

(use-modules (srfi srfi-1))

(define (part1 input)
  (let* ((lines (input->lines input))
         (numbers (map line->number-list lines))
         (symbols (map line->symbol-list lines))
         (numbers-kept (numbers-next-to-sym numbers (cons '() symbols))))
...
(define input "............830..743.......59..955.......663..........................................367...........895....899...............826...220......
.......284.....*............*.....$...+.....*...377..................*.......419.............488...*.......*...................*..-....939..
